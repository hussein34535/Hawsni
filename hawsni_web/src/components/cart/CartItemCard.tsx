...
export default function CartItemCard({ item }: { item: CartItem }) {
    ...
    return (
        <motion.div
            ...
            className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-50 flex items-center gap-6 relative"
        >
            {/* Image (Right in RTL, Left in LTR) */}
            <div className={`w-32 h-32 rounded-[2rem] overflow-hidden flex-shrink-0 bg-gray-50 ${isRTL ? 'order-2' : 'order-1'}`}>
