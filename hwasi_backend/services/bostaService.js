...
// Source: GET /api/v0/cities + manual Arabic aliases from Hwasi addresses
...
class BostaService {
    ...
    async createShipment(orderData, options = {}) {
        try {
            ...
            let description = productNames.length > 0 ? productNames.join(' + ') : `Hwasi Order #${orderData.order_number || orderData.id}`;
            ...
            const payload = {
                ...
                receiver: {
                    ...
                    lastName: customerName.split(' ').slice(1).join(' ') || 'Hwasi',
