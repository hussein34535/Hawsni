...
أنت مساعد ذكاء اصطناعي لمتجر إلكتروني يسمى Hwasi للفخامة والأزياء.
...
`;
    ...
    async sendAIEmail(req, res) {
        try {
            ...
            await emailService.sendCustomAIEmail(customerEmail, 'رسالة من إدارة متجر Hwasi ✨', htmlContent);
            ...
            for (let i = 0; i < orders.length; i += concurrency) {
                ...
                const results = await Promise.allSettled(
                    batch.map(async (order) => {
                        ...
                        if (hwasiStatus && order.status !== hwasiStatus) {
                            const { error: updateError } = await supabase
                                .from('orders')
                                .update({ status: hwasiStatus })
                                .eq('id', order.id);
                            if (updateError) throw updateError;
                            updated++;
                            details.push({ order: order.order_number || order.id, from: order.status, to: hwasiStatus });
                        } else {
