            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-4">
                    ...
                    <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-gray-100">
                        {images[selectedImage]?.match(/\.(mp4|webm|mov|ogg)($|\?)/i) || images[selectedImage]?.includes('video/upload') ? (
                            <div className="relative w-full h-full">
                                <video
                                    src={optimizeVideoUrl(images[selectedImage])}
                                    ...
                                    autoPlay
                                    muted
                                    onLoadStart={() => setVideoLoadingStates(prev => ({ ...prev, [selectedImage]: true }))}
                                    onPlaying={() => setVideoLoadingStates(prev => ({ ...prev, [selectedImage]: false }))}
                                    onCanPlay={() => setVideoLoadingStates(prev => ({ ...prev, [selectedImage]: false }))}
                                />
                                {videoLoadingStates[selectedImage] && (
                    ...
                    {images.length > 1 && (
                        <div className="flex gap-3 overflow-x-auto pb-2">
                            {images.map((img, index) => {
                                const isVid = img?.match(/\.(mp4|webm|mov|ogg)($|\?)/i) || img?.includes('video/upload');
                                return (
                                    <button
                                        key={index}
